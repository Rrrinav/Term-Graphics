...
public:
                        ...
                        Color maxColor)
  {
    ...
    {
      position = position + utl::Vec<float, 2>{static_cast<float>(rand()) / RAND_MAX * 2 - 1, static_cast<float>(rand()) / RAND_MAX * 2 - 1};  // Random position
...
int main()
{
  ...
  while (true)
  {
    ...
    if (Window::has_mouse_moved())
    {
      ...
      if (mouse.event == Mouse_event_type::RIGHT_CLICK)
        particleSystem.create_explosion({(float)mouse.x, (float)mouse.y}, 100, 10.0f, 2.0f, 5.0f,RED, BLUE);  // Adjust parameters as needed
    }
