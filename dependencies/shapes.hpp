...
public:
  Shape() : _color(utl::Color_codes::WHITE) {}
...
public:
  ...
  Line(int x1, int y1, int x2, int y2, char ch, Color color = utl::Color_codes::WHITE)
      : Shape(color), _start({x1, y1}), _end({x2, y2}), _ch(ch)
  {
  }
...
public:
  ...
  char get_char() const { return _ch; }
  float get_area() const { return std::abs((_v1[0] * (_v2[1] - _v3[1]) + _v2[0] * (_v3[1] - _v1[1]) + _v3[0] * (_v1[1] - _v2[1])) / 2.0f); }
...
public:
  ...
  float get_area() const
  {
    ...
    for (size_t i = 0; i < n; ++i) area += (_vertices[i][0] * _vertices[(i + 1) % n][1]) - (_vertices[(i + 1) % n][0] * _vertices[i][1]);
